// AI 서버 함수 (Netlify Functions)
// - mode 없음: 발달상황 기록 초안 '다듬기'
// - mode:"keyword": 입력 키워드에 어울리는 발달상황 관찰 문장 한 줄 '생성'
// API 키는 Netlify 환경변수 ANTHROPIC_API_KEY 에서 읽어옵니다. 코드에 직접 넣지 마세요.
exports.handler = async (event) => {
  if (event.httpMethod !== "POST") return { statusCode: 405, body: "Method Not Allowed" };
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) return json(500, { error: "API 키가 설정되지 않았어요. Netlify 환경변수 ANTHROPIC_API_KEY 를 확인해 주세요." });
  let body;
  try { body = JSON.parse(event.body || "{}"); } catch { return json(400, { error: "잘못된 요청이에요" }); }
  body = body || {};

  if (body.mode === "keyword") {
    if (!body.keyword) return json(400, { error: "키워드가 없어요" });
    const r = await generate({ key, keyword: body.keyword, age: body.age, domain: body.domain, style: body.style });
    return json(r.status, r.body);
  }
  if (!body.text) return json(400, { error: "다듬을 내용이 없어요" });
